...
init_bias_min_in_sim = 1000
min_in_sim = 10000 + init_bias_min_in_sim

# Use the alternate simulation queue selection strategy
reverse_priority_strategy = True

# Use the round robin simulation queue selection strategy
round_robin = False

# Use common service times for different queue selection strategies (Be sure to have generated the times using
# Milestone4GenerateServiceTimes.py).
use_common_service_times = True

# Generate data for milestone 3 (milestone 2 data is on another branch titled crushton/milestone2-data-gen)
generate_milestone3_data = False

# Generate data for milestone 4
generate_milestone4_data = False

# Print service times used by each workstation and inspector and all states generated by numpy to sample states
# use_common_service_times must be False
print_st_state_data = False
...
sim_speed_factor = 10000  # TODO: A couple values like products produced depend on this which shouldn't, but the ratios are still correct
...
monitor.use_common_service_times = use_common_service_times
...
mean_component1_service_time = find_mean(open('servinsp1.dat').read().splitlines())
mean_component2_service_time = find_mean(open('servinsp22.dat').read().splitlines())
mean_component3_service_time = find_mean(open('servinsp23.dat').read().splitlines())
mean_workstation1_service_time = find_mean(open('ws1.dat').read().splitlines())
mean_workstation2_service_time = find_mean(open('ws2.dat').read().splitlines())
mean_workstation3_service_time = find_mean(open('ws3.dat').read().splitlines())
c1_st_file_name = "./common_service_times/component1.dat"
c2_st_file_name = "./common_service_times/component2.dat"
c3_st_file_name = "./common_service_times/component3.dat"
w1_st_file_name = "./common_service_times/workstation1.dat"
w2_st_file_name = "./common_service_times/workstation2.dat"
w3_st_file_name = "./common_service_times/workstation3.dat"


def convert_st_mins_to_sim_speed(irl_st_minutes):
    return irl_st_minutes * 60 / sim_speed_factor
...
# The order matters! Element 0 has highest priority
workstations = [
    Workstation(my_type=Type.ONE,
                my_buffers=[Buffer(Type.ONE)],
                mean_st=mean_workstation1_service_time,
                st_file_name=w1_st_file_name),
    Workstation(my_type=Type.TWO,
                my_buffers=[Buffer(Type.ONE), Buffer(Type.TWO)],
                mean_st=mean_workstation2_service_time,
                st_file_name=w2_st_file_name),
    Workstation(my_type=Type.THREE,
                my_buffers=[Buffer(Type.ONE), Buffer(Type.THREE)],
                mean_st=mean_workstation3_service_time,
                st_file_name=w3_st_file_name)
]
if reverse_priority_strategy:
    workstations = [
        Workstation(my_type=Type.THREE,
                    my_buffers=[Buffer(Type.ONE), Buffer(Type.THREE)],
                    mean_st=mean_workstation3_service_time,
...
inspector_one = Inspector(known_workstations=workstations, seed=seed, my_types=[Type.ONE],
                          mean_st_components={
                              Type.ONE: mean_component1_service_time
                          },
                          st_file_names={
                              Type.ONE: c1_st_file_name
                          },
                          round_robin=round_robin)
inspector_two = Inspector(known_workstations=workstations, seed=seed, my_types=[Type.TWO, Type.THREE],
                          mean_st_components={
                              Type.TWO: mean_component2_service_time,
                              Type.THREE: mean_component3_service_time
                          },
                          st_file_names={
                              Type.TWO: c2_st_file_name,
                              Type.THREE: c3_st_file_name
                          },
                          round_robin=round_robin)
...
def calculate_performance(monitor):
    ...
    logger.info("Reverse priority strategy: %s", reverse_priority_strategy)
    logger.info("Round Robin Strategy: %s", round_robin)
    logger.info("Common service times: %s", use_common_service_times)
    ...
    ######################## THROUGHPUT ########################
    p1_throughput_per_hour = monitor.products_made[Type.ONE] / data_collection_time * 60
    p2_throughput_per_hour = monitor.products_made[Type.TWO] / data_collection_time * 60
    p3_throughput_per_hour = monitor.products_made[Type.THREE] / data_collection_time * 60
    total_products_made = monitor.products_made[Type.ONE] + monitor.products_made[Type.TWO] + monitor.products_made[Type.THREE]
    total_throughput_per_hour = total_products_made / data_collection_time * 60
    logger.info("Factory throughput (products/hour) for product one: %s", p1_throughput_per_hour)  # Multiply by 60 to convert mins to hours
    logger.info("Factory throughput (products/hour) for product two: %s", p2_throughput_per_hour)
    logger.info("Factory throughput (products/hour) for product three: %s", p3_throughput_per_hour)
    logger.info("Factory total throughput (products/hour): %s", total_throughput_per_hour)
    ...
    if generate_milestone4_data:
        system = "sys3"
        if reverse_priority_strategy:
            system = "sys2"
        print_single_data_to_file("./m4_data/block_time_inspector1_" + system +".dat", i1_prop_block * 100)
        print_single_data_to_file("./m4_data/block_time_inspector2_" + system + ".dat", i2_prop_block * 100)
        print_single_data_to_file("./m4_data/product1_produced_" + system + ".dat", monitor.products_made[Type.ONE])
        print_single_data_to_file("./m4_data/product2_produced_" + system + ".dat", monitor.products_made[Type.TWO])
        print_single_data_to_file("./m4_data/product3_produced_" + system + ".dat", monitor.products_made[Type.THREE])
        print_single_data_to_file("./m4_data/throughput_product1_" + system + ".dat", p1_throughput_per_hour)
        print_single_data_to_file("./m4_data/throughput_product2_" + system + ".dat", p2_throughput_per_hour)
        print_single_data_to_file("./m4_data/throughput_product3_" + system + ".dat", p3_throughput_per_hour)

        print_single_data_to_file("./m4_data/total_block_time_" + system + ".dat", i1_prop_block * 100 + i2_prop_block * 100)
        print_single_data_to_file("./m4_data/total_products_produced_" + system + ".dat", total_products_made)
        print_single_data_to_file("./m4_data/total_throughput_" + system + ".dat", total_throughput_per_hour)


def print_single_data_to_file(file_name, data):
